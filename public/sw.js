// MatricPrep AI - Service Worker
// Enables offline functionality and caching

const CACHE_NAME = 'matricprep-ai-v1';
const urlsToCache = [
  '/',
  '/index.html',
  '/manifest.json',
...
async function syncProgress() {
  try {
    ...
    if (progress && progress.length > 0) {
      // Here you would sync with backend if needed
...
async function updateSchedule() {
  try {
    // Update study schedule periodically
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification('MatricPrep AI', options)
  );
});
