...
// Use Vercel AI Gateway (recommended) or direct OpenAI API
const openai = createOpenAI({
  apiKey: process.env.OPENAI_API_KEY || process.env.AI_GATEWAY_API_KEY,
  baseURL: process.env.AI_GATEWAY_BASE_URL || undefined,
})

// Subject-specific knowledge base for better responses
const SUBJECT_GUIDES: Record<string, string> = {
  Physics:
    'Explain concepts using formulas, real-world examples, and include step-by-step problem solving. Cover mechanics, heat, light, electricity, and modern physics.',
  ...
  Chemistry:
    'Explain chemical reactions, atomic structure, bonding, and stoichiometry. Use balanced equations and relate to practical applications.',
  Computer:
    'Cover programming concepts, data structures, algorithms, and databases. Provide code examples when relevant to the Pakistani curriculum.',
  English:
    'Help with grammar, vocabulary, essay writing, and comprehension. Provide context from Pakistani literature and examples.',
  Urdu:
    'Explain Urdu grammar, literature, poetry, and writing skills. Use proper Urdu script and cultural context where applicable.',
}

export async function POST(request: Request) {
  try {
    const body = await request.json()
    const { message, subject, studentName } = body
