...
export default function StudentOnboarding({ onComplete }: StudentOnboardingProps) {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md bg-card rounded-md shadow-sm border border-border p-6 md:p-8">
        {/* Header */}
              ...
              <div className="grid grid-cols-2 gap-3">
                {SUBJECTS.map((subj) => (
                  <button
                    ...
                    className={`p-3 rounded-md border-2 transition-all font-medium text-sm ${
          ...
          <div className="flex gap-3 mt-8">
            {step > 1 && (
              <button
                ...
                className="flex-1 px-4 py-3 rounded-md border border-border text-foreground font-medium hover:bg-muted transition-colors"
              >
                Back
              </button>
            )}
